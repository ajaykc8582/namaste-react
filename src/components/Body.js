    ...
    return listOfRestaurants.length === 0 ? (<Shimmer />) : (
        <div className="body">
            ...
            <div className="flex flex-wrap">
                {
                    filteredRestaurant.map((restaurant) => <Link to={"/restaurants/" + restaurant.info.id} key={restaurant.info.id} style={{ textDecoration: 'none' }}><RestaurantCard resData={restaurant} /></Link>)
                }
            </div>
        </div>
    )
